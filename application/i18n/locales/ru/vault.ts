...
export const ruVaultMessages: Messages = {
  ...
  'pf.action.releaseRemotePort': 'Освободить удалённый порт и запустить',
  'pf.release.remoteConfirmTitle': 'Освободить старый удалённый туннель',
  'pf.release.remoteConfirmDesc': 'Netcatty подключится к SSH-серверу, завершит старый дочерний процесс sshd, который держит удалённый порт {port} для правила "{label}", и запустит туннель снова.',
  'pf.release.remoteConfirmAction': 'Освободить и запустить',
